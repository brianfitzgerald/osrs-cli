...
| `loadout <url-or-id> [--json]` | Read equipment, stats, prayers, boosts, combat settings, and the target from a shared OSRS Wiki DPS Calculator link. |
...
1. **Get current state.** If the goal touches quests or anything quest-gated, run `full <user>`. Otherwise `player <user>` is cheaper. Do this once up front — reuse the output rather than re-querying.
   - If the user supplies a DPS Calculator share link, run `loadout <url-or-id> --json` once and use it as the equipment source. The payload describes configured loadouts, not the player's bank or inventory.
...
- **DPS share scope.** A share payload contains configured loadouts and calculator settings. Treat those items as the proposed setup, not proof that every item is currently owned.
